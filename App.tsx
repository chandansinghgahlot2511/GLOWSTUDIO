...
const App: React.FC = () => {
  ...
  const handlePromptSubmit = async (prompt: string, referenceImage: ImageFile | null) => {
    ...
    try {
      ...
      if (isSafetyError) {
        setErrorMsg("Request cannot be processed due to safety guidelines.");
      } else {
        setErrorMsg("Something went wrong while processing the image.");
      }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-purple-500/30">
      ...
      <div className="flex justify-center w-full px-4 md:px-8 gap-6 max-w-[1920px] mx-auto relative">
        ...
        {/* Central Content */}
        <main className="flex-1 w-full max-w-6xl mx-auto py-12 pb-40 min-w-0 transition-all duration-300">
            ...
            <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fadeIn">
              <div className="text-center mb-10 max-w-2xl">
                <h2 className="text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400 mb-6 leading-tight">
                  Reimagine your photos with GlowStudio
                </h2>
                <p className="text-lg text-slate-400">
                  Powered by Gemini 2.5 Flash Image. Upload, Filter, and Transform.
                </p>
              </div>
          ...
          {(status !== AppStatus.IDLE && originalImage && workingImage) && (
            <div className="animate-fadeIn">
               <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-semibold text-white">Editor Workspace</h3>
                    ...
                    <div className={`relative group rounded-2xl overflow-hidden border border-slate-800 bg-slate-900 shadow-xl min-h-[300px] flex items-center justify-center ${status === AppStatus.ERROR ? 'border-red-500/30' : ''}`}>
                       {status === AppStatus.PROCESSING ? (
                          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/80 backdrop-blur-sm z-20">
                             ...
                             <p className="text-purple-400 animate-pulse font-medium">Gemini is dreaming...</p>
                          </div>
                       ) : null}
                            ...
                            <div className="text-center p-10 text-slate-500">
                              {status === AppStatus.ERROR ? (
                                <div className="text-red-400 opacity-80">
                                  ...
                                  <p>{errorMsg || "Something went wrong while processing the image."}</p>
                                </div>
                              ) : (
                                <>
